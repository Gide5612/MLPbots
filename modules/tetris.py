...
class Tetris(Cog):
    ...
    @command(description="3", name="tet", help="Сыграть в Тетрис", brief="Упоминание пользователя",
             usage="tet @Принцесса Луна")
    async def tet(self, ctx, member: Member = None):
        try:
            await ctx.message.delete(delay=1)
            if member is not None:
                user, e = DB.server.tetris.find_one({"_id": member.id}), None
                if user is not None:
                    e = Embed(title="Статистика игры \"Тетрис\":", color=ctx.author.color,
                              description=f"**Пользователь {member.mention}:**\n"
                                          f"Сыграно игр: **{user['Сыграно игр']}**\n"
                                          f"Лучший счет: **{user['Лучший счет']}**")
                else:
                    e = Embed(title="Статистика игры \"Тетрис\":", color=ctx.author.color,
                              description=f"У {member.mention} еще нет статистики...")
                db, top, i = DB.server.tetris.find().sort("Лучший счет", DESCENDING), "", 1
                for user in db:
                    if i <= 10:
                        top += f"<@{user['_id']}>: {user['Лучший счет']}\n"
                    i += 1
                    ...
                    while True:
                        ...
                        for x in range(5):
                            ...
                            if column == 0:
                                ...
                                user = DB.server.tetris.find_one({"_id": member.id})
                                if user is None:
                                    DB.server.tetris.insert_one({"_id": int(member.id), "Имя": member.name,
                                                                 "Сыграно игр": int(1), "Лучший счет": int(score)})
                                else:
                                    DB.server.tetris.update_one({"_id": int(member.id)},
                                                                {"$inc": {"Сыграно игр": int(1)}})
                                    if score > user["Лучший счет"]:
                                        DB.server.tetris.update_one({"_id": int(member.id)},
                                                                    {"$set": {"Лучший счет": int(score)}})
                                for xx in range(5):
                                    for xxx in range(5):
